...
function GetRolesFromMicrosoftGraph() {
    Write-Host "Retrieving Directory Role Templates from Microsoft Graph"
    # $tenantIdList = @("f8cdef31-a31e-4b4a-93e4-5f571e91255a", "72f988bf-86f1-41af-91ab-2d7cd011db47", "cdc5aeea-15c5-4db6-b079-fcadd2505dc2")
    $select = "Id,displayName"

    $roles = Get-MgDirectoryRoleTemplate -Select $select -All
    # foreach ($tenantId in $tenantIdList) {
    #     $filter = "appOwnerOrganizationId eq $($tenantId)"
    #     $servicePrincipals += Get-MgServicePrincipal -Filter $filter -Select $select -ConsistencyLevel eventual -PageSize 999 -CountVariable $count -All
    # }
...
$roleList += GetRolesFromMicrosoftGraph
...
$outputFilePathRolesCsv = Join-Path $OutputPath "DirectoryRoles.csv"
$outputFilePathRolesJson = Join-Path $OutputPath "DirectoryRoles.json"
...
$roleList | Export-Csv $outputFilePathRolesCsv
$roleList | ConvertTo-Json | Out-File $outputFilePathRolesJson